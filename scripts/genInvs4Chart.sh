...
genInvs() {
	...
	for testCls in "${testClses[@]}"; do
		...
		ranTests+=( $class )
		cmd="java -Xmx10240M -cp lib/*:build/:build-tests/:$DAIKONDIR/daikon.jar daikon.DynComp --decl-file=TestRunner${count}.decls-DynComp junit.textui.TestRunner ${class}"
		echo "$cmd"
		eval "$cmd"
		cmd="java -Xmx10240M -cp lib/*:build/:build-tests/:$DAIKONDIR/daikon.jar daikon.Chicory --comparability-file=TestRunner${count}.decls-DynComp --dtrace-file=TestRunner${count}.dtrace.gz junit.textui.TestRunner ${class}"
		echo "$cmd"
		eval "$cmd"
		cmd="java -Xmx10240M -cp lib/*:build/:build-tests/:$DAIKONDIR/daikon.jar daikon.Daikon TestRunner${count}.dtrace.gz"
		echo "$cmd"
		eval "$cmd"
		count=$((count+1))
	done
